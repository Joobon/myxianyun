<template>
    <div class="main">
        ...
        <div class="air-column">
            <h2>保险</h2>
            <div>
                <div class="insurance-item" v-for="(item,index) in data.insurances" :key="index" @change="bingo(item.id)">
                    <el-checkbox 
...
export default {
    ...
    computed: {
        allprice(){
            let price=0
            let Nums=0
            // if(this.users.length===1&&this.users.username==='') return  
            if(!this.data.seat_infos.org_settle_price) return 
            //数量
            Nums=this.users.length
            // 机票钱
            ...
            this.$store.commit('air/setAllNum',Nums)
            return "" 
        }
    },
    methods: {
