...
export default {
  ...
  methods: {
    ...
    handleSearchTab(item, index) {
      if (index === 1) {
        this.$message.warning("暂未开通往返功能");
      }
    },
    // 出发城市失去焦点时，默认选中第一项
    handleDepartBlur() {
      if (this.departArr.length>0) {
    ...
    queryDepartSearch(value, callback) {
      ...
      this.$axios({
        ...
        this.departArr = data.map(v => {
          v.value = v.name.replace("市", "");
          return v;
        });

        callback(this.departArr);
      })
    },
    ...
    queryDestSearch(value, callback) {
      ...
      this.$axios({
        ...
        // 循环给data中每一项添加一个value属性，并且没有市字的
        this.destArr = data.map(v => {
          v.value = v.name.replace("市", "");
          return v;
        });

        callback(this.destArr);
      });
    },
